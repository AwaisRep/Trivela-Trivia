...
export default defineComponent({
    ...
    setup() {
        ...
        const fetchGameDetails = async () => {
            ...
            const url = `${import.meta.env.VITE_API_BASE_URL}/career_path/game/${gameID}`; // In development this is localhost
        ...
        const guess = async () => {
            if (gameDetails.value !== null) {
                ...
                const url = `${import.meta.env.VITE_API_BASE_URL}/career_path/guess/${gameID}`;
                const csrfToken = document.cookie.split('; ').find(row => row.startsWith('csrftoken='))?.split('=')[1];
